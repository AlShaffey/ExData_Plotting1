...
rstudioapi::restartSession('result <- draw_4(); result')
  ...
  augment_plot <- function()
  {
    ...
    plot_3 <- function()
    {
      ...
      for (i in seq_along(y)) {
        annotations[i] <- ifelse(i %% 2 > 0, as.character(y[i]), "") 
      }
      
      axis(side = 2, lwd = 2, lwd.ticks = 2, at = y, labels = annotations)
    ...
    plot_4 <- function()
    {
      ...
      axis(side = 2, lwd = 2, lwd.ticks = 2, at = c(0, .1, .2, .3, .4, .5))
